(function() {
  var CoffeeScript, Package, async, compilers, eco, extname, fs, join, less, normalize, _, _ref,
  ...
  try {
    ...
    compilers.less = function(module, filename) {
      ...
      return less.render(content, function(e, content) {
        ...
        return module._compile("s = document.createElement('style'); s.innerHTML = \"" + content + "\"; document.head.appendChild(s); ", filename);
      });
    };
  } catch (err) {
  ...
  try {
    compilers.css = function(module, filename) {
      var content;
      content = fs.readFileSync(filename, 'utf8');
      content = content.replace(/["]/g, "'");
      content = content.replace(/[\r]/g, "");
  ...
  try {
    compilers.json = function(module, filename) {
      ...
      return module._compile("if (!window.JSONImport) window.JSONImport = {}; window.JSONImport['" + name + "'] = '" + content + "'; ");
    };
  } catch (error) {
